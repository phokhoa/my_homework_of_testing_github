...
import FCNetwork_unified as network_unified
#import FCNetwork_dropout as network
...
def myKMeans_eachSubClass(trn, trn_label, tst, tst_label,num_label,group):

	numclusters = 36
	centroids_subclass = []
	for i in range(num_label):
		datai = [x for x, l in zip(trn, trn_label) if l == i]
		kmeans = KMeans(n_clusters=numclusters, random_state=0).fit(datai)
		centroids_subclass.append(kmeans.cluster_centers_)

	predict = []
	for t in trn:
		dist = []
		for i in range(num_label):
			dist.append(np.min([mydistance(t, cs) for cs in centroids_subclass[i]]))
		dist = np.array(dist)
		predict.append(np.argmin(dist))
...
def myKMeans_eachSubClass_(trn, trn_label, tst, tst_label,num_label,group,trn_subclass_label,centroids_subclass,centroids_subclass_group):
...
def LDA_eachSubClass_(trn, trn_label, tst, tst_label,num_label,group,trn_subclass_label,centroids_subclass,centroids_subclass_group):
	clf = LinearDiscriminantAnalysis()
	clf.fit(trn, trn_subclass_label)

	predict = unify_label(clf.predict(trn),centroids_subclass_group)
	trn_acc = np.sum(predict == trn_label) / (1.0 * len(predict))
	trn_acc_unified = np.sum(unify_label(predict, group) == unify_label(trn_label, group)) / (1.0 * len(predict))

	predict = unify_label(clf.predict(tst),centroids_subclass_group)
	tst_acc = np.sum(predict == tst_label) / (1.0 * len(predict))
	tst_acc_unified = np.sum(unify_label(predict, group) == unify_label(tst_label, group)) / (1.0 * len(predict))

	return trn_acc, trn_acc_unified, tst_acc, tst_acc_unified

def KNN_eachSubClass_(trn, trn_label, tst, tst_label,num_label,group,trn_subclass_label,centroids_subclass,centroids_subclass_group):
	n_neighbor = 5
...
def SVM_eachSubClass_(trn, trn_label, tst, tst_label,num_label,group,trn_subclass_label,centroids_subclass,centroids_subclass_group):
	clf = svm.LinearSVC()
	clf.fit(trn, trn_subclass_label)

	predict = predict = unify_label(clf.predict(trn),centroids_subclass_group)
	trn_acc = np.sum(predict == trn_label) / (1.0 * len(predict))
	trn_acc_unified = np.sum(unify_label(predict, group) == unify_label(trn_label, group)) / (1.0 * len(predict))
...
def FCNetwork_eachSubClass_(trn, trn_label, tst, tst_label,num_label,group,trn_subclass_label,centroids_subclass,centroids_subclass_group):
	num_output = np.max(trn_subclass_label)+1
	trn_label_one_hot = [vectorized_result(l,num_output).reshape(-1) for l in trn_subclass_label]
	sizes = [trn.shape[1], 500,500, num_output ]
	#net = network.FCNetwork(sizes,0.5)
	net = network.FCNetwork(sizes)

	predict = unify_label(np.argmax(net.feedforward(trn),axis = 1),centroids_subclass_group)
	trn_acc = np.sum(predict == trn_label) / (1.0 * len(predict))
	trn_acc_unified = np.sum(unify_label(predict, group) == unify_label(trn_label, group)) / (1.0 * len(predict))

	predict = unify_label(np.argmax(net.feedforward(tst), axis = 1),centroids_subclass_group)
	tst_acc = np.sum(predict == tst_label) / (1.0 * len(predict))
	tst_acc_unified = np.sum(unify_label(predict, group) == unify_label(tst_label, group)) / (1.0 * len(predict))
	...
	for _i in range(200):
		...
		net.SGD([(x,y) for x,y in zip(trn, trn_label_one_hot)], 1, 36, lr_rate, 0.02)
		#print net.biases[0].get_value()
...
def FCNetwork(trn, trn_label, tst, tst_label,num_label,group,trn_subclass_label,centroids_subclass,centroids_subclass_group):
	num_output = num_label
	trn_label_one_hot = [vectorized_result(l,num_output).reshape(-1) for l in trn_label]
	sizes = [trn.shape[1], 500,500, num_output ]
	#net = network.FCNetwork(sizes,0.5)
	net = network.FCNetwork(sizes)

	predict = np.argmax(net.feedforward(trn),axis = 1)
	trn_acc = np.sum(predict == trn_label) / (1.0 * len(predict))
	trn_acc_unified = np.sum(unify_label(predict, group) == unify_label(trn_label, group)) / (1.0 * len(predict))

	predict = np.argmax(net.feedforward(tst),axis = 1)
	tst_acc = np.sum(predict == tst_label) / (1.0 * len(predict))
	tst_acc_unified = np.sum(unify_label(predict, group) == unify_label(tst_label, group)) / (1.0 * len(predict))
	print trn_acc, trn_acc_unified, tst_acc, tst_acc_unified
	...
	lr_rate = 0.2
	for _i in range(200):
		net.SGD([(x,y) for x,y in zip(trn, trn_label_one_hot)], 1, 36, lr_rate, 0.002)
		predict = np.argmax(net.feedforward(trn),axis = 1)
		trn_acc = np.sum(predict == trn_label) / (1.0 * len(predict))
		trn_acc_unified = np.sum(unify_label(predict, group) == unify_label(trn_label, group)) / (1.0 * len(predict))
...
def FCNetwork_unified(trn, trn_label, tst, tst_label,num_label,group,trn_subclass_label,centroids_subclass,centroids_subclass_group):
	#num_output = num_label
	trn_label_one_hot = [vectorized_result(l,num_label).reshape(-1) for l in trn_label]
	trn_label_unified_one_hot = [vectorized_result(l,len(group)).reshape(-1) for l in unify_label(trn_label,group)]
	sizes = [trn.shape[1], 500,500, num_label, len(group) ]
	#net = network.FCNetwork(sizes,0.5)
	net = network_unified.FCNetwork(sizes)
	...
	lr_rate = 0.2
	for _i in range(200):
		net.SGD([(x,y,y_unified) for x,y,y_unified in zip(trn, trn_label_one_hot,trn_label_unified_one_hot)], 1, 36, lr_rate, 0.002)
		predict = np.argmax(net.feedforward(trn),axis = 1)
		trn_acc = np.sum(predict == trn_label) / (1.0 * len(predict))
		trn_acc_unified = np.sum(unify_label(predict, group) == unify_label(trn_label, group)) / (1.0 * len(predict))
...
def LDA(trn, trn_label, tst, tst_label,num_label,group):
	...
	return trn_acc, trn_acc_unifiednum_label, tst_acc, tst_acc_unified
...
def get_centroid_label(trn,trn_label,numlabel):
	#get labels for centroids in each subclass
	numclusters = 36
	centroids_subclass = []
	kmeans_subclass = []
	for _i in range(numlabel):
		datai = [x for x,l in zip(trn,trn_label) if l==_i]
		kmeans =  KMeans(n_clusters=numclusters,random_state=0).fit(datai)
		centroids_subclass.append(kmeans.cluster_centers_)
		kmeans_subclass.append(kmeans)
	sc_l = 0
	centroids_subclass_group = []
	for _cs in centroids_subclass:
		_cs_group = []
		for _c in _cs:
			_cs_group.append(sc_l)
			sc_l += 1
		centroids_subclass_group.append(_cs_group)

	subclass_centroids_labels = [centroids_subclass_group[_l][int(kmeans_subclass[_l].predict([_x]))] for _x,_l in zip(trn,trn_label)]

	return subclass_centroids_labels, centroids_subclass,centroids_subclass_group
...
ratio_test = 0.6
degree_step = 10
methods = ['FCNetwork_unified']
#methods = ['myKMeans_eachSubClass']
...
for fold in range(num_k_folds):
		...
		for xlarge in xlarges:
				...
				for pca_keep in pca_keeps:
						...
						"""
						#(trn, trn_label, tst, tst_label,num_label,group,trn_subclass_label,centroids_subclass,centroids_subclass_group):
						trn_projected_sc_centroids_labels, trn_projected_centroids_sc,trn_projected_centroids_sc_group = get_centroid_label(X_train_projected,train_sample_labels,num_label)
						for method in methods:
							print method + 'started'
							print num_label
							trn_acc, trn_acc_unified, tst_acc, tst_acc_unified = eval(method)(X_train_projected, train_sample_labels,
																					  test_samples_projected, test_labels,
																					  num_label,group,
																					  trn_projected_sc_centroids_labels,trn_projected_centroids_sc,trn_projected_centroids_sc_group)
							print method + 'ended'
							temp_foldres.append([trn_acc, trn_acc_unified, tst_acc, tst_acc_unified])
						...
						trn_sc_centroids_labels, trn_centroids_sc,trn_centroids_sc_group = get_centroid_label(train_samples,train_sample_labels,num_label)
						for method in methods:
							print method + 'nopca started'
							trn_acc, trn_acc_unified, tst_acc, tst_acc_unified = eval(method)(train_samples, train_sample_labels,
																					  test_samples, test_labels,
																					  num_label,group,
																					  trn_sc_centroids_labels,trn_centroids_sc,trn_centroids_sc_group)
							print method + 'nopca ended'
							temp_foldres.append([trn_acc, trn_acc_unified, tst_acc, tst_acc_unified])
						"""
...
with open('outfile_kmeans_for_each_subclass_allfold_40_FCNet_','wb') as fp:
	pickle.dump(final_res,fp)
with open('outfile_kmeans_for_each_subclass_40_FCNet_','wb') as fp:
	pickle.dump(ave_final_res,fp)
with open('outfile_kmeans_for_each_subclass_40_FCNet_', 'rb') as fp:
