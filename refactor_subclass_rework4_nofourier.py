...
def get_sample_part2(X,xsmall, xlarge):
    ...
    for magnitude_spectrum, l, cur_pos in X:
        ...
        f_ishift_filtered = np.fft.ifftshift(highpass)
        img_back = np.fft.ifft2(f_ishift_filtered)
        img_back = np.abs(img_back)
        img_back = cv2.resize(img_back, (60,60))
        ...
        XX.append((img_back.reshape(-1), l, cur_pos))
    return np.array(XX)
"""
#for vstack
def get_sample_part2(X,xsmall, xlarge):
...
with open('outfile_rework_4_allfold_40_nofourier','wb') as fp:
    pickle.dump(final_res,fp)
with open('outfile_rework_4_40_nofourier','wb') as fp:
    pickle.dump(ave_final_res,fp)
with open('outfile_rework_4_40_nofourier', 'rb') as fp:
