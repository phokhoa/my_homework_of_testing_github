...
# In[5]:
def ceil_to_nearest(a,base):
    _ia = int(a)
    _ibase = int(base)
    if _ia % _ibase == 0:
        return a
    else:
        return (_ia/_ibase +1.0)*_ibase

def floor_to_nearest(a,base):
    _ia = int(a)
    _ibase = int(base)
    if _ia % _ibase == 0:
        return a
    else:
        return (_ia/_ibase)*_ibase
...
print sz
#sz = [(floor_to_nearest(_sz[0],HoG_pixels_per_cell[1]),floor_to_nearest(_sz[1],HoG_pixels_per_cell[0])) for _sz in sz]
#print sz
...
max_scales = []
min_scales = []
for _sz,_spos in zip(sz,spos):
	max_scale = 0.0
	min_scale = INF
	for _pos in _spos:
		s0 = _pos[2].shape[0] / _sz[0]
		s1 = _pos[2].shape[1] / _sz[1]
		max_s = max(s0,s1)
		min_s = min(s0,s1)
		if min_scale > min_s:
			min_scale = min_s
...
def SVM(trn, trn_label,num_label,group):
    numpos_per_label = []
    ...
    clf = svm.SVC(class_weight=weight,kernel='rbf',probability=True)
    clf.fit(trn, trn_label)
...
def get_dense_HardNegatives(sz,spos,_idx, neg_images,negs_per_pos):
	res = []
	fsize = sz[_idx]
	for _spos in spos:
		for _pos in _spos:
			pos = _pos[2]
			for _k in range(negs_per_pos):
				neg_image = getNegatives(neg_images,fsize,1)[0]
				res.append(get_one_HardNegative_by_rescale(fsize, pos,neg_image))
				res.append(get_one_HardNegative_by_translate(fsize,pos))
	return res
...
def extract_pos(pos, fsize):
    ...
    #S_f = fsize[0]*fsize[1]
    #S_pos = cur_h*cur_w
    #scale = 0.9*np.sqrt(S_f/S_pos)
    pos_resize = cv2.resize(cur_pos,(int(0.9*fsize[1]),int(0.9*fsize[0])))
    resize_h, resize_w = pos_resize.shape
    #print cur_h, cur_w, resize_h,resize_w,fsize[0], fsize[1]
    cx = int(fsize[1]/2)
    cy = int(fsize[0]/2)
    #print cx,cy, resize_h/2,resize_w/2,cy-resize_h/2,cy+resize_h/2
    mask = np.zeros((int(fsize[0]),int(fsize[1])))
    mask[cy-resize_h/2:cy+(resize_h+1)/2,cx-resize_w/2:cx+(resize_w+1)/2] = pos_resize
    res.append(mask)

    pos_resize2 = cv2.resize(cur_pos,(int((1/0.9)*fsize[1]),int((1/0.9)*fsize[0])))
    resize_h, resize_w = pos_resize2.shape
    cx = int(resize_w/2)
    cy = int(resize_h/2)
    mask = pos_resize2[cy-int(fsize[0])/2:cy+int(fsize[0])/2, cx-int(fsize[1])/2:cx+int(fsize[1])/2]
    res.append(mask)
...
for _idx in range(len(spos)):
    ...
    start_time = time.time()
    negs = []
    for neg in getNegatives(neg_images,_sz,len(_sp)):
        negs.append(neg)
...
for _sp, comp_para in zip(spos, comp_parameters):
    ...
    for trn_sample in sp_train_samples_projected:
        reconstructed.append(reconstruct_from_pca(trn_sample,comp_ux,comp_sx,comp_vx,comp_datamean, comp_datamax,comp_k))
