...
with open("myModel_improved.pickle",'rb') as f:
    classifier,names,sz,ux,sx,vx,datamean, datamax, k,comp_parameters,root_filters,max_errors,min_norms,xlarge,xsmall,HoG_pixels_per_cell,HoG_cells_per_block = pickle.load(f)
uxx = ux[:,:k]
"""
...
def nms_multiclass(hypotheses, overlap, numlabel):
    ...
    _c = np.array([c for x,y,w,h,c in hypotheses])
...
print detections
with open('result_improved.pickle','wb') as f:
	pickle.dump(detections,f)
"""
#with open('result_improved.pickle','rb') as f:
...
for _c in range(len(names)):
	...
	if not len(prec) == 0:
		...
		mAP.append((ap,0,0))
print mAP
with open('mAP_improved.pickle','wb') as f:
	pickle.dump(mAP,f)
